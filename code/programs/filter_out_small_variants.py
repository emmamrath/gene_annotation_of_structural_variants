#!/usr/bin/python
# python filter_out_small_variants.py sample_id
# cat test_SV.vcf | python filter_out_small_variants.py -f 49 > test_SV_large.vcf

# This program removes VCF records whose size is smaller than the given size.
...
# If there are multiple BND alleles, then if any of them are larger than the given size, the VCF record is kept.
...
def main():
	...
	parser = argparse.ArgumentParser(description='Filter out structural variants smaller than given size')
	parser.add_argument('-f', action="store", dest="filter_length", required=True, help='filter to discard SVs whose length (INFO.END,INFO.SVLEN.INFO.MEINFO.END is smaller than or equal to this filter length')
			...
			if ((got_length == False) and (is_an_insertion == False)):
				...
				for this_alt in this_alts_fields:
					...
					if (got_this_alt_length):
						...
						highest_alt_length = max(this_alt_length, highest_alt_length)
				if (got_at_least_one_alt_length):
					got_length = True
					this_length = highest_alt_length
